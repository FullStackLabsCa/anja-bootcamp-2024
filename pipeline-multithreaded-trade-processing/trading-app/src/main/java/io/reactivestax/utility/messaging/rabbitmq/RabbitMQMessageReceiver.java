...
public class RabbitMQMessageReceiver {
    ...
    private static final int RETRY_TTL = 5000;
    ...
    public Channel getReceiverChannel(String queueName) throws IOException, TimeoutException {
        ...
        channel.queueDeclare(ApplicationPropertiesUtils.getInstance().getRetryQueueName(), true, false, false, retryArgs);
        channel.queueBind(ApplicationPropertiesUtils.getInstance().getRetryQueueName(), EXCHANGE_NAME, "retry");
        Map<String, Object> mainQueueArgs = new HashMap<>();
        mainQueueArgs.put("x-dead-letter-exchange", EXCHANGE_NAME);
        mainQueueArgs.put("x-dead-letter-routing-key", "retry");
